...
// CODE HERE
// sayHelloButton.className = 'hello-button';
...
sayHelloButton.addEventListener('mouseout', () => {
    sayHelloButton.className = 'hello-button-off';
})
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/Kim-Rocks')
        .then(res => {
            return res.data
        });
...
// CODE HERE
const queryFunction = () => {
    axios.get('http://localhost:3000/repeat?query-test')
        .then(res => {
            return res.data
        });
}
